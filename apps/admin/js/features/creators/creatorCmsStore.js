...
const CREATOR_CMS_SCHEMA_VERSION = 2;

export async function hydrateCreatorsFromCms(){
    const local = getCreators();
    let access;
    try{access = await getCmsAccessState();}catch(error){console.warn("[cms] Creator hydrate fell back to local cache", error);return local;}
    if(!access.configured || !access.authenticated || !access.isAdmin) return local;
    let rows = await listCmsCreators();
    const initialized = rows.some(row => Number(row?.profile?.adminSchemaVersion) >= 1);
    if(!initialized){await persistCreatorsToCms(local, rows);rows = await listCmsCreators();}
...
export async function saveCreatorsCanonical(collection){
    ...
    if(!access.isAdmin) throw new Error("RELMUA CMSのAdmin権限がありません。System > Databaseで権限を確認してください。");
    const rows = await listCmsCreators();
    await persistCreatorsToCms(normalized, rows);
    return writeCreatorCache(createCollectionFromCms(await listCmsCreators()));
}

export async function addCreatorCanonical(creator){
    const collection = getCreators();
    const created = normalizeCreator({ ...creator, order: nextOrder(collection.creators) }, { touchTimestamps: true });
    await saveCreatorsCanonical({ ...collection, creators: [...collection.creators, created] });
    return created;
}
...
export async function updateCreatorCanonical(id, updates){
    ...
    creators[index] = normalizeCreator({ ...current, ...updates, id: current.id, createdAt: current.createdAt }, { touchUpdatedAt: true });
    await saveCreatorsCanonical({ ...collection, creators });
    return true;
}
...
export async function deleteCreatorCanonical(id){
    ...
    if(creators.length === collection.creators.length) return false;
    await saveCreatorsCanonical({ ...collection, creators });
    return true;
}
...
export async function moveCreatorCanonical(id, direction){
    ...
    await saveCreatorsCanonical({ ...collection, creators: creators.map((creator, order) => ({ ...creator, order: order + 1 })) });
    return true;
}

export async function setPrimaryCreatorCanonical(id){
    const collection = getCreators();
    if(!collection.creators.some(creator => creator.id === id)) return false;
    await saveCreatorsCanonical({ ...collection, primaryCreatorId: id });
    return true;
}
...
export async function resolveCmsCreatorId(legacyId){
    ...
    const rows = await listCmsCreators();
    return rows.find(row => row.legacy_id === target && row.status !== "archived")?.id || null;
}

export function createCollectionFromCms(rows){
    const activeRows = (Array.isArray(rows) ? rows : []).filter(row => row && row.status !== "archived").slice().sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
    ...
    const collection = normalizeCreatorsCollection({ primaryCreatorId: primary?.legacy_id || creators[0]?.id || "", creators });
...
async function persistCreatorsToCms(collection, existingRows=[]){
    ...
    const existingByLegacyId = new Map(rows.map(row => [String(row.legacy_id || ""), row]));
    const desiredIds = new Set(normalized.creators.map(creator => creator.id));
    for(const row of rows){if(row?.legacy_id && !desiredIds.has(row.legacy_id) && row.status !== "archived") await archiveCreatorByLegacyId(row.legacy_id);}
    const nonPrimary = normalized.creators.filter(creator => creator.id !== normalized.primaryCreatorId);
    const primary = normalized.creators.find(creator => creator.id === normalized.primaryCreatorId);
    for(const creator of nonPrimary) await upsertCreatorByLegacyId(toCmsCreator(creator, false, existingByLegacyId.get(creator.id)));
    if(primary) await upsertCreatorByLegacyId(toCmsCreator(primary, true, existingByLegacyId.get(primary.id)));
}
...
function writeCreatorCache(collection){
    const normalized = normalizeCreatorsCollection(collection);
    validateCreatorsCollection(normalized);
    if(save(CREATORS_KEY, normalized) === false) throw new Error("Creatorsのローカルcacheを更新できませんでした");
    return normalized;
}
function nextOrder(creators){return creators.reduce((max,creator)=>Math.max(max,Number(creator.order)||0),0)+1;}
