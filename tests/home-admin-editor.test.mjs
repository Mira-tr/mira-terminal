...
test("Home Editor remains available from the Admin Dashboard", async () => {
    ...
    assert.match(dashboard, /href:\s*"\.\/home\/"/);
    assert.doesNotMatch(dashboard, /HOME_CONFIG_KEY|normalizeHomeConfig|validateHomeConfig|saveHomeConfig|loadHomeConfig/);
    assert.match(studioApp, /home:\s*Object\.freeze\(/);
    assert.match(studioApp, /mount:\s*mountHomeEditor/);
    assert.doesNotMatch(studioApp, /createWorkspaceItem\("ホーム", "\.\.\/admin\/home\/", "active"\)/);
...
test("Home Admin Editor uses Home Store API and only adds Public Export", async () => {
    ...
    assert.match(html, /homeGuideTitle/);
    assert.match(html, /ここでできること/);
    assert.match(html, /表示エリア/);
...
test("Home Form keeps section id and type fixed and hides Hero-only irrelevant fields", async () => {
    ...
    assert.match(form, /panel\.id\s*=\s*`home-section-\$\{section\.id\}`/);
    assert.match(form, /createMeta\("場所",\s*section\.id\)/);
    assert.match(form, /createMeta\("種類",\s*TYPE_LABELS\[section\.type\]/);
    assert.match(form, /FIELD_LABELS/);
    assert.match(form, /表示する内容/);
    assert.match(form, /新しい順で自動表示/);
